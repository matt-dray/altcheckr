...
```{r get}
webpage <- "https://alphagov.github.io/accessibility-tool-audit/test-cases.html#images"
get_img <- alt_get(url = webpage)
```
...
You should pass the output of `alt_get()` to `alt_check()` to perform a series of
basic assessments for the presence and 'quality' of each image's alt text.

You can also pass any data frame that contains a `src` and `alt` column, where
`alt` contains the text to be assessed by `alt_check()`.
...
Each new column is the outcome of a check for an possible accessibility issue
